...
- (instancetype)initWithTextLanguage:(NSString *)originalTextField updatedTextField:(NSString *)updatedTextField zoomLevel:(NSNumber *)zoomLevel;

@end

@implementation __MGLTextLanguage
...
@property (nonatomic) NS_MUTABLE_DICTIONARY_OF(NSString *, NS_DICTIONARY_OF(NSObject *, __MGLTextLanguage *) *) *localizedLayersByIdentifier;
...
- (void)setLocalizesLabels:(BOOL)localizesLabels
{
    ...
    if (_localizesLabels) {
        ...
        for (MGLSymbolStyleLayer *layer in self.layers) {
            ...
            if ([layer.text isKindOfClass:[MGLConstantStyleValue class]]) {
                ...
                if (![textField isEqualToString:localizingString]) {
                    __MGLTextLanguage *textLanguage = [[__MGLTextLanguage alloc] initWithTextLanguage:textField
        ...
        [self.localizedLayersByIdentifier enumerateKeysAndObjectsUsingBlock:^(NSString *identifier, NSDictionary<NSObject *,__MGLTextLanguage *> *textFields, BOOL *done) {
            MGLSymbolStyleLayer *layer = (MGLSymbolStyleLayer *)[self.mapView.style layerWithIdentifier:identifier];
            
            if ([layer.text isKindOfClass:[MGLConstantStyleValue class]]) {
                NSString *textField = [(MGLConstantStyleValue<NSString *> *)layer.text rawValue];
                [textFields enumerateKeysAndObjectsUsingBlock:^(NSObject *originalLanguage, __MGLTextLanguage *textLanguage, BOOL *done) {
                ...
                MGLCameraStyleFunction *function = (MGLCameraStyleFunction<NSString *> *)layer.text;
                NSMutableDictionary *stops = function.stops.mutableCopy;
                [textFields enumerateKeysAndObjectsUsingBlock:^(NSObject *zoomKey, __MGLTextLanguage *textLanguage, BOOL *done) {
